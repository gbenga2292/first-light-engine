...
import { LayoutDashboard, Package, Plus, FileText, ShoppingCart, Settings, MapPin, PlusCircle, LogOut, LogIn, Activity, Sun, Moon, RefreshCw } from "lucide-react";
...
export const Sidebar = ({
  ...
  return <div className={cn("bg-card shadow-soft flex flex-col transition-all duration-300", mode === 'desktop' ? "w-64 border-r border-border h-full" : "w-full h-full border-none")}>
      ...
      {isElectron && syncStatus && <div className="p-3 border-t border-border">
          <Button variant="ghost" size="sm" className={cn("w-full justify-start gap-2 h-auto py-2 flex-col items-start", syncStatus.inSync && syncStatus.status === 'synced' && "opacity-50 cursor-default")} onClick={handleManualSync} disabled={isSyncing || syncStatus.inSync && syncStatus.status === 'synced'}>
            <div className="flex items-center gap-2 w-full">
              <RefreshCw className={cn("h-3.5 w-3.5", isSyncing && "animate-spin")} />
              <span className="text-xs font-medium">Sync Now</span>
            </div>
            {syncStatus.lastSync && <span className="text-[10px] text-muted-foreground">
                Last: {formatDistanceToNow(new Date(syncStatus.lastSync), {
            addSuffix: true
          })}
              </span>}
          </Button>
